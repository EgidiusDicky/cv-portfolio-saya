...
const getFullImageUrl = (relativePath) => {
    // In development, use localhost. In production (on Vercel), use the current origin.
    ...
    <div class="pt-16 px-4 max-w-6xl mx-auto text-white">
        ...
        <div v-else-if="project">
            ...
            <div class="relative bg-gray-200 rounded-lg overflow-hidden mb-6">
                <!-- Wrap image in an anchor tag to make it clickable -->
                <a v-if="project.images && project.images.length > 0"
                   :href="getFullImageUrl(project.images[currentImage])" <!-- CHANGED HERE -->
                   class="block"
                >
                    <img :src="getFullImageUrl(project.images[currentImage])" <!-- CHANGED HERE -->
                ...
                <!-- Previous Image Button (with SVG) - Only show if more than one image -->
                <button
                    v-if="project.images && project.images.length > 1"
                    @click.stop="prevImage"
                    class="absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black p-2 rounded-full text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-75"
                >
                ...
                <!-- Next Image Button (with SVG) - Only show if more than one image -->
                <button
                    v-if="project.images && project.images.length > 1"
                    @click.stop="nextImage"
                    class="absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black p-2 rounded-full text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-75"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Image Counter - Only show if images exist -->
            ...
            <div class="flex flex-wrap gap-2 mb-10">
                <span v-for="tag in project.tags" :key="tag" class="bg-gray-700 px-3 py-1 rounded-full text-sm">{{ tag }}</span>
            </div>

            <!-- Info Cards -->
            <div class="space-y-6 mb-12">
                <div v-for="section in project.sections" :key="section.title" class="bg-gray-800 p-6 rounded-lg shadow">
                    <h2 class="text-xl font-semibold text-yellow-400 mb-2">{{ section.title }}</h2>
            ...
            <!-- Project Links -->
            <div class="mb-12">
                <h2 class="text-2xl font-bold text-center text-yellow-400 mb-6">Project Links</h2>
        ...
        <!-- Contact (Footer) - Added here for ProjectDetail.vue -->
        <Contact />
    </div>
</template>
